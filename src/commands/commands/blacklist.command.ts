...
export class BlacklistCommand extends AdminCommand {
  ...
  data = new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) => {
      ...
      Object.values(BlacklistCommand.GET_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      subcommand.setName("add").setDescription("Blacklist users and roles");
      Object.values(BlacklistCommand.ADD_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      subcommand.setName("delete").setDescription("Whitelist users and roles");
      Object.values(BlacklistCommand.DELETE_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    });
  ...
  static readonly GET_OPTIONS = {
    queues: new QueuesOption({
      required: true,
      description: "Get blacklisted entries of specific queue(s)",
    }),
  };

  static async blacklist_get(
    inter: SlashInteraction,
    queues?: Collection<bigint, DbQueue>
  ) {
    queues = queues ?? (await BlacklistCommand.GET_OPTIONS.queues.get(inter));

    const blacklisted = inter.store
      .dbBlacklisted()
      .filter((blacklisted) => queues.has(blacklisted.queueId));
  ...
  static readonly ADD_OPTIONS = {
    queues: new QueuesOption({
      required: true,
      description: "Queue(s) to blacklist from",
    }),
  ...
  static async blacklist_add(inter: SlashInteraction) {
    ...
    const { updatedQueueIds, insertedBlacklisted } =
      await BlacklistUtils.insertBlacklisted(
        inter.store,
        queues,
        mentionables,
        reason
    ...
    await inter.respond(
      `Blacklisted ${mentionablesMention(
        insertedBlacklisted
      )} from the '${queuesMention(updatedQueues)}' queue${
        updatedQueues.length > 1 ? "s" : ""
      }.`,
      true
    );
    await this.blacklist_get(
      inter,
  ...
  static async blacklist_delete(inter: SlashInteraction) {
    const blacklisteds = await BlacklistCommand.DELETE_OPTIONS.blacklisteds.get(
      inter
    );

    const { deletedBlacklisted, updatedQueueIds } =
      BlacklistUtils.deleteBlacklisted(
        inter.store,
        blacklisteds.map((blacklisted) => blacklisted.id)
      );

    const blacklistedStr = deletedBlacklisted
      .map((blacklisted) => {
