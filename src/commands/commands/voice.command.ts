...
export class VoiceCommand extends AdminCommand {
  ...
  data = new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) => {
      subcommand.setName("get").setDescription("Get voice integrations");
      Object.values(VoiceCommand.GET_OPTIONS).forEach((option) =>
    ...
    .addSubcommand((subcommand) => {
      ...
      Object.values(VoiceCommand.ADD_SOURCE_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      ...
      Object.values(VoiceCommand.SET_SOURCE_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      ...
      Object.values(VoiceCommand.DELETE_SOURCE_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      subcommand
        ...
        .setDescription(
          "Set voice destination channel (also possible with /queues set)"
        );
      Object.values(VoiceCommand.SET_DESTINATION_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      subcommand
        ...
        .setDescription(
          "Reset voice destination channel (also possible with /queues reset)"
        );
      Object.values(VoiceCommand.DISABLE_DESTINATION_OPTIONS).forEach(
        (option) => option.addToCommand(subcommand)
      );
      return subcommand;
    });
  ...
  static readonly ADD_SOURCE_OPTIONS = {
    queues: new QueuesOption({
      required: true,
      description: "Queue(s) to set source voice channel",
    }),
    sourceVoiceChannel: new VoiceSourceChannelOption({
      required: true,
      description: "Voice channel to pull members from",
    }),
    joinSync: new JoinSyncToggleOption({
      description: "Toggle whether members are enqueued on voice join",
    }),
    leaveSync: new LeaveSyncToggleOption({
      description: "Toggle whether members are dequeued on voice leave",
    }),
  };
  ...
  static async voice_add_source(inter: SlashInteraction) {
    ...
    const sourceChannelId =
      VoiceCommand.ADD_SOURCE_OPTIONS.sourceVoiceChannel.get(inter)?.id;
    ...
    const [voiceOnlyQueues, nonVoiceOnlyQueues] = partition(
      [...queues.values()],
    ...
    for (const queue of voiceOnlyQueues.values()) {
      ...
      if (members.size) {
        const confirmed = await inter.promptConfirmOrCancel(
          `The '${queueMention(queue)}' queue has ${inlineCode(
        ...
        if (!confirmed) {
          await inter.respond(
            "Cancelled voice integration addition. No changes have been made."
          );
          return;
        }
      }
    ...
    if (nonVoiceOnlyQueues.length) {
      await inter.respond(
        `${inlineCode(
          VoiceOnlyToggleOption.ID
        )} is not enabled for the '${queuesMention(nonVoiceOnlyQueues)}' queue${
          nonVoiceOnlyQueues.length > 1 ? "s" : ""
        }. ` +
          `Members will still be able to join via buttons or commands. ${inlineCode(
            VoiceOnlyToggleOption.ID
          )} can be changed with ${commandMention("queues", "set")}.`
      );
    }

    const newVoice = { sourceChannelId, joinSync, leaveSync };

    const { updatedQueueIds } = VoiceUtils.insertVoices(
      inter.store,
      queues,
      newVoice
    );
    ...
    await inter.respond(
      `Added voice integrations to the '${queuesMention(updatedQueues)}' queue${
        updatedQueues.length > 1 ? "s" : ""
      }`,
      true
    );
    await this.voice_get(
      inter,
  ...
  static readonly SET_SOURCE_OPTIONS = {
    voices: new VoicesOption({
      required: true,
      description: "Voice integrations to update",
    }),
    voiceSourceChannel: new VoiceSourceChannelOption({
      description: "Voice channel to pull members from",
    }),
    joinSync: new JoinSyncToggleOption({
      description: "Toggle whether members are enqueued on voice join",
  ...
  static async voice_set_source(inter: SlashInteraction) {
    ...
    const voiceSourceChannelId =
      VoiceCommand.SET_SOURCE_OPTIONS.voiceSourceChannel.get(inter)?.id;
    ...
    const { updatedQueueIds } = VoiceUtils.updateVoices(
      inter.store,
    ...
    await inter.respond(
      `Updated voice integrations in '${queuesMention(updatedQueues)}' queue${
        updatedQueues.length > 1 ? "s" : ""
      }`,
      true
    );
  ...
  static readonly DELETE_SOURCE_OPTIONS = {
    voices: new VoicesOption({
      required: true,
      description: "Voice integrations to delete",
    }),
  };
  ...
  static async voice_delete_source(inter: SlashInteraction) {
    ...
    const { updatedQueueIds } = VoiceUtils.deleteVoices(
      inter.store,
      voices.map((voice) => voice.id)
    );
    const updatedQueues = updatedQueueIds.map((id) =>
    ...
    await inter.respond(
      `Deleted voice integrations in '${queuesMention(updatedQueues)}' queue${
        updatedQueues.length > 1 ? "s" : ""
      }`,
      true
    );
  ...
  static readonly SET_DESTINATION_OPTIONS = {
    queues: new QueuesOption({
      required: true,
      description: "Queue(s) to set destination voice channel",
    }),
  ...
  static async voice_set_destination(inter: SlashInteraction) {
    ...
    const voiceDestinationChannelId =
      VoiceCommand.SET_DESTINATION_OPTIONS.voiceDestinationChannel.get(
        inter
      )?.id;

    await QueueUtils.updateQueues(inter.store, queues, {
      voiceDestinationChannelId,
    });

    await inter.respond(
      `Updated voice destination channel of '${queuesMention(queues)}' queue${
        queues.size > 1 ? "s" : ""
      }.`,
      true
    );
  ...
  static readonly DISABLE_DESTINATION_OPTIONS = {
    queues: new QueuesOption({
      required: true,
      description: "Queue(s) to disable destination voice channel",
    }),
  };

  static async voice_disable_destination(inter: SlashInteraction) {
    const queues = await VoiceCommand.DISABLE_DESTINATION_OPTIONS.queues.get(
      inter
    );

    await QueueUtils.updateQueues(inter.store, queues, {
      voiceDestinationChannelId: null,
    });

    await inter.respond(
      `Unset voice destination channel of '${queuesMention(queues)}' queue${
        queues.size > 1 ? "s" : ""
      }.`,
      true
    );
