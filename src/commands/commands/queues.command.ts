...
export class QueuesCommand extends AdminCommand {
  ...
  data = new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) => {
      subcommand.setName("get").setDescription("Get queues properties");
      Object.values(QueuesCommand.GET_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      subcommand.setName("add").setDescription("Create a queue");
      Object.values(QueuesCommand.ADD_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      subcommand.setName("set").setDescription("Set queue properties");
      Object.values(QueuesCommand.SET_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      subcommand.setName("reset").setDescription("Reset queue properties");
      Object.values(QueuesCommand.RESET_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      subcommand.setName("delete").setDescription("Delete a queue");
      Object.values(QueuesCommand.DELETE_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    });
  ...
  static readonly GET_OPTIONS = {
    queues: new QueuesOption({
      required: true,
      description: "Get specific queue(s)",
    }),
  };

  static async queues_get(
  ...
  static readonly ADD_OPTIONS = {
    ...
    autopullToggle: new AutopullToggleOption({
      description: "Toggle automatic pulling of queue members",
    }),
    badgeToggle: new BadgeToggleOption({
      description: "Toggle badges next to queue name",
    }),
    color: new ColorOption({ description: "Color of the queue" }),
    displayButtons: new DisplayButtonsOption({
      description: "Toggle buttons beneath queue displays",
    }),
    displayUpdateType: new DisplayUpdateTypeOption({
      description: "How to update displays",
    }),
    header: new HeaderOption({ description: "Header of the queue display" }),
    inlineToggle: new InlineToggleOption({
      description: "Toggle inline display of queue members",
    }),
    lockToggle: new LockToggleOption({
      description: "Toggle queue locked status (prevents joining)",
    }),
    memberDisplayType: new MemberDisplayTypeOption({
      description: "How to display members",
    }),
    notificationsToggle: new NotificationsToggleOption({
      description: "Toggle whether users are DM-ed on pull",
    }),
    pullBatchSize: new PullBatchSizeOption({
      description: "How many queue members to include in a pull",
    }),
    pullMessage: new PullMessageOption({
      description: "Additional message to include on pull",
    }),
    rejoinCooldownPeriod: new RejoinCooldownPeriodOption({
      description:
        "# of seconds a member must wait before re-queueing after being pulled",
    }),
    rejoinGracePeriod: new RejoinGracePeriodOption({
      description:
        "# of seconds a member has to reclaim their queue spot after leaving",
    }),
    roleInQueue: new RoleInQueueOption({
      description: "Role to assign members of the queue",
    }),
    roleOnPull: new RoleOnPullOption({
      description: "Role to assign members when they are pulled",
    }),
    size: new SizeOption({ description: "Limit the size of the queue" }),
    timestampType: new TimestampTypeOption({
      description: "Display timestamp format",
    }),
    voiceOnlyToggle: new VoiceOnlyToggleOption({
      description:
        "Toggle whether queue is restricted to members in source voice channel",
    }),
    voiceDestinationChannel: new VoiceDestinationChannelOption({
      description: "Voice channel to move members to when they are pulled",
    }),
  };
  ...
  static async queues_add(inter: SlashInteraction) {
      ...
      ...omitBy(
        {
          ...
          displayUpdateType:
            QueuesCommand.ADD_OPTIONS.displayUpdateType.get(inter),
          ...
          memberDisplayType:
            QueuesCommand.ADD_OPTIONS.memberDisplayType.get(inter),
          notificationsToggle:
            QueuesCommand.ADD_OPTIONS.notificationsToggle.get(inter),
          ...
          rejoinCooldownPeriod:
            QueuesCommand.ADD_OPTIONS.rejoinCooldownPeriod.get(inter),
          rejoinGracePeriod:
            QueuesCommand.ADD_OPTIONS.rejoinGracePeriod.get(inter),
          ...
          voiceDestinationChannelId:
            QueuesCommand.ADD_OPTIONS.voiceDestinationChannel.get(inter)?.id,
        },
        isNil
      ),
    };
    ...
    await DisplayUtils.insertDisplays(
      inter.store,
      [insertedQueue],
      inter.channelId
    );

    await QueuesCommand.queues_get(
      inter,
      toCollection<bigint, DbQueue>("id", [insertedQueue])
    );
  }
  ...
  static readonly SET_OPTIONS = {
    ...
    size: new SizeOption({ description: "Limit the size of the queue" }),
    timestampType: new TimestampTypeOption({
      description: "Display timestamps format",
    }),
    voiceOnlyToggle: new VoiceOnlyToggleOption({
      description:
        "Toggle whether queue is restricted to members in source voice channel",
  ...
  static async queues_set(inter: SlashInteraction) {
    ...
    const update = omitBy(
      {
        ...
        displayUpdateType:
          QueuesCommand.SET_OPTIONS.displayUpdateType.get(inter),
        ...
        memberDisplayType:
          QueuesCommand.SET_OPTIONS.memberDisplayType.get(inter),
        name: QueuesCommand.SET_OPTIONS.name.get(inter),
        notificationsToggle:
          QueuesCommand.SET_OPTIONS.notificationsToggle.get(inter),
        ...
        rejoinCooldownPeriod:
          QueuesCommand.SET_OPTIONS.rejoinCooldownPeriod.get(inter),
        rejoinGracePeriod:
          QueuesCommand.SET_OPTIONS.rejoinGracePeriod.get(inter),
    ...
    if (update.voiceOnlyToggle) {
      const nonVoiceOnlyQueues = queues.filter(
        (queue) => !queue.voiceOnlyToggle
      );
      for (const queue of nonVoiceOnlyQueues.values()) {
        const members = inter.store
          .dbMembers()
          .filter((member) => member.queueId === queue.id);
        if (members.size) {
          const confirmed = await inter.promptConfirmOrCancel(
            `You are enabling ${inlineCode(
              VoiceOnlyToggleOption.ID
            )} for the '${queueMention(queue)}' queue. ` +
              `There are ${members.size} member${
                members.size === 1 ? "" : "s"
              } in the '${queueMention(
                queue
              )}' queue that will be cleared if you proceed. ` +
              "Do you wish to proceed?"
          );
          if (!confirmed) {
            await inter.respond(
              "Cancelled queue update. No changes have been made."
            );
            return;
          }
        }
      ...
      for (const queue of nonVoiceOnlyQueues.values()) {
        ...
        await inter.respond(
          `Cleared ${queueMention(queue)} queue of members due to ${inlineCode(
            VoiceOnlyToggleOption.ID
          )} being enabled.`,
          true
        );
    ...
    const { updatedQueues } = await QueueUtils.updateQueues(
      inter.store,
      queues,
      update
    );
    ...
    await inter.respond(
      `Updated ${Object.keys(update)
        .map(propertyMention)
        .join(", ")} of '${queuesMention(queues)}' queue${
        queues.size > 1 ? "s" : ""
      }.`,
      true
    );
    ...
    await QueuesCommand.queues_get(
      inter,
      toCollection<bigint, DbQueue>("id", updatedQueues)
    );
  }
  ...
  static async queues_reset(inter: SlashInteraction) {
    ...
    const selectMenuOptions = [
      ...
      {
        name: DisplayUpdateTypeOption.ID,
        value: QUEUE_TABLE.displayUpdateType.name,
      },
      ...
      { name: LockToggleOption.ID, value: QUEUE_TABLE.lockToggle.name },
      {
        ...
        value: QUEUE_TABLE.memberDisplayType.name,
      },
      { name: NameOption.ID, value: QUEUE_TABLE.name.name },
      {
        name: NotificationsToggleOption.ID,
        value: QUEUE_TABLE.notificationsToggle.name,
      },
      ...
      {
        name: RejoinCooldownPeriodOption.ID,
        value: QUEUE_TABLE.rejoinCooldownPeriod.name,
      },
      {
        name: RejoinGracePeriodOption.ID,
        value: QUEUE_TABLE.rejoinGracePeriod.name,
      },
      ...
      {
        name: VoiceOnlyToggleOption.ID,
        value: QUEUE_TABLE.voiceOnlyToggle.name,
      },
    ];
    ...
    const propertiesToReset = await selectMenuTransactor.sendAndReceive(
      "Queue properties to reset",
      selectMenuOptions
    );

    const updatedProperties = {} as any;
    for (const property of propertiesToReset) {
      const columnKey = findKey(
        QUEUE_TABLE,
        (column: SQLiteColumn) => column.name === property
      );
    ...
    if (updatedProperties.roleId) {
      await MemberUtils.assignInQueueRoleToMembers(
        inter.store,
        queues,
        updatedProperties.roleId,
        "remove"
      );
    }

    const propertiesStr = propertiesToReset.map(inlineCode).join(", ");
    const propertiesWord =
      propertiesToReset.length === 1 ? "property" : "properties";
    ...
    await selectMenuTransactor.updateWithResult(
      `Reset ${queuesWord} ${propertiesWord}`,
      resetPropertiesStr
    );
  ...
  static async queues_delete(inter: SlashInteraction) {
    ...
    const confirmed = await inter.promptConfirmOrCancel(
      `Are you sure you want to delete the '${queueMention(queue)}' queue?`
    );
    if (!confirmed) {
    ...
    await inter.respond(
      `Deleted the '${queueMention(deletedQueue)}' queue.`,
      true
    );
  }
}
